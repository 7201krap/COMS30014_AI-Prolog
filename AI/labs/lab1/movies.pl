...
%------------------------------------------------------------------------------%
    % a. released_after(M, Y) <- the movie was released after the given year.
    % released_after(M, 2000). 를 입력하면?
    % released_after(M, 2000)이 참이 되게 하는 goal 을 찾음.
    % goal: 'movie(M,Y2), 2000<Y2.' 이 참이 되도록 하는 M 을 나열 한다.
    released_after(M,Y) :- movie(M,Y2), Y<Y2.
...
% co_direct(D1,D2).	D1 and D2 have co-directed some movie together
co_direct(M, D1,D2) :-
  director(M, D1), director(M, D2), D1 @> D2.

% self_direct(M,D).	D played in a movie M which they also directed
self_direct(M,D) :-
  director(M, D), plays(M, D, _).

% cameo(M,A).	A played them self in movie M
cameo(M, A):-
  plays(M, A, A); actor(M, A, himself); actress(M, A, herself).
...
veteran(A) :-
  plays(M1, A, _), plays(M2, A, _),
  movie(M1, Y1), movie(M2, Y2),
  Y1-Y2>25.
